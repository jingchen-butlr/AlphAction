...
__global__ void RoIAlign3dForward(const int nthreads, const T* bottom_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    // We do average (integral) over all ROI bins
...
__global__ void RoIAlign3dBackwardFeature(const int nthreads, const T* top_diff,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    int top_offset = (n * channels + c) * length * pooled_height * pooled_width;
    const T* offset_top_diff = top_diff + top_offset;
    const T top_diff_this_bin = offset_top_diff[(l * pooled_height + ph) * pooled_width + pw];
...
at::Tensor ROIAlign3d_forward_cuda(const at::Tensor& input,
  ...
  auto output = at::zeros({num_rois, channels, length, pooled_height, pooled_width}, input.options());

  auto output_size = num_rois * pooled_height * pooled_width * length * channels;
